...
class HistoryCompatibleUSEquityAdjustmentReader(object):
    ...
    def load_adjustments(self, columns, dates, assets, perspective_offset=0):
        """
        Returns
        -------
        adjustments : list[dict[int -> Adjustment]]
            A list of mappings from index to adjustment objects to apply at
            that index.
        """
        ...
        for i, column in enumerate(columns):
            ...
            for asset in assets:
                adjs.update(self._get_adjustments_in_range(
                    asset, dates, column, perspective_offset))
            out[i] = adjs
        return out
        ...
        if field != 'volume':
            ...
            for m in mergers:
                ...
                if start < dt <= end:
                    ...
                    adj_loc = end_loc - perspective_offset
...
class HistoryLoader(with_metaclass(ABCMeta)):
    ...
    def _ensure_sliding_windows(self, assets, dts, field,
        ...
        if needed_assets:
            ...
            perspective_offset = int(is_perspective_after)

            for i, asset in enumerate(needed_assets):
                if self._adjustments_reader:
                    adjs = self._adjustments_reader.load_adjustments(
                        [field], prefetch_dts, [asset], perspective_offset)[0]
                else:
                    adjs = {}
